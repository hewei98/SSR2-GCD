...
class OxfordFlowers(OxfordFlowers_Base):
    def __init__(self, retrieved_text_path, root=None, split='train', transform=None, target_transform=None, text_transform=None, download=True, num_tags=0, num_attributes=0):
    ...
    def __getitem__(self, item):
        ...
        if self.text_transform is not None:
            base_text = self.text_transform(base_text)
            base_token = [self.safe_tokenize(t) for t in base_text]
        else:
            base_token = self.safe_tokenize(base_text)
        ...
        for i, tag in enumerate(tags):
            ...
            if self.text_transform is not None:
                tag = self.text_transform(tag)
                token = [self.safe_tokenize(t) for t in tag]
            else:
                token = self.safe_tokenize(tag)
        
            tag_token.append(token)
        ...
        for i, attr in enumerate(attributes):
            ...
            if self.text_transform is not None:
                attr = self.text_transform(attr)
                token = [self.safe_tokenize(t) for t in attr]
            else:
                token = self.safe_tokenize(attr)

            attr_token.append(token)

        return img, label, str(uq_idx), base_token, tag_token, attr_token, uq_idx
