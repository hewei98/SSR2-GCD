'''
We follow TextGCD (https://arxiv.org/pdf/2403.07369v1)
and its code (https://github.com/HaiyangZheng/TextGCD) 
to search prompt candidates from tag and attribute lexions.
'''
import os
import csv
import torch
...
def generate_tags_attributes_for_batches(dataloader, text_generator, output_file, idx_file, start_idx=0, num_tags=5, num_attributes=5):
    all_results = []  
    for batch_idx, samples in enumerate(tqdm(dataloader, desc="Processing batches", initial=start_idx, total=len(dataloader))):
        with torch.no_grad():
            # set numbers of candidates for RTG
            outputs = text_generator(samples, num_tags=num_tags, num_attributes=num_attributes)
            batch_results = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--auxiliary_model_name', default='hf-hub:laion/CLIP-ViT-B-16-laion2B-s34B-b88K', type=str)
    ...
    parser.add_argument('--num_attributes', default=5, type=int)
    ...
    dataset = RTG_dataset_funcs[args.dataset_name](root=dataset_root, transform=sample_transform, download=True) # , download=True
    sample_dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)
